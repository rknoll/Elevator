 ...
 * <ul>
 * <li><i>Elevator capacity.</i> The maximum number of passengers that can fit on an
 * elevator.</li>
 * <li><i>Elevator speed.</i> The maximum speed an elevator can travel at in feet/sec.
 * It is assumed to be the same for all elevators.</li>
 ...
 * <ul>
 * <li><i>Door opening and closing.</i> Elevator doors automatically open when arriving at a floor and
 * close before departing. When passengers are waiting to board an elevator, the doors will 
 * stay open until all passengers are boarded. Each open and close action takes 1 second.</li>
 * <li><i>Elevator buttons.</i> Arriving passengers press the appropriate elevator button and the
 * light is automatically turned off when the elevator arrives at the associated floor.</li>
 * <li><i>Elevator speed and acceleration.</i> The elevator will automatically accelerate to the maximum
 * speed possible until there is only enough time left to slow down to arrive at the target floor.</li>
 * </ul>
 * 
 * <p>There is substantial elevator status information available:
 * <ul>
 * <li><i>Elevator speed.</i> The current speed of the elevator. Positive speed indicates an elevator
 * heading up while negative indicates an elevator going down.</li>
 * <li><i>Elevator acceleration.</i> The acceleration of the elevators.</li>
 * <li><i>Elevator button status.</i> For each elevator, which floor buttons on the elevator have
 * been pressed.</li>
 * <li><i>Elevator position.</i> The current position of the elevator, both in feet and to
 * the closest floor. Position is measured in feet above ground floor with zero being the bottom.</li>
 * <li><i>Elevator door status.</i> Whether the doors for an elevator are open or closed. The
 * status may also indicate a transition between open or closed.</li>
 * <li><i>Elevator target.</i> The current floor target of the elevator as set by the controller. The
 * elevator will travel to that target and stop until directed to the next target.</li>
 * <li><i>Elevator committed direction.</i> The current committed direction of the elevator. Elevators
 ...
 * to passenger travel..</li>
 * <li><i>Elevator floor service.</i> Whether a particular elevator services a particular floor. When
 * elevators are allowed to only service certain floors, this can help to achieve greater passenger 
 * service. Every elevator must service the ground floor.</li>
 * <li><i>Elevator weight.</i> This provides the current weight of the elevator less the weight of the
 * empty elevator - hence the weight of the passengers on board. This can be useful for detecting
 * when the elevator is getting full.</li>
 * </ul>
 * 
 * <p>The elevators can be controlled by setting the following parameters:
 * <ul>
 * <li><i>Elevator target.</i> Sets the floor target of a particular elevator. Typically, this target
 ...
 * next target.</li>
 * <li><i>Elevator committed direction.</i> Sets the current committed direction of the elevator. This
 ...
 * dispatched to a target floor, it should have a committed direction set.</li>
 * <li><i>Elevator floor service.</i> Sets whether a particular elevator services a particular floor.
 * In larger buildings, it is typically better to set elevator service to a particular subset of floors. 
 * Note that the ground floor will always get service.</li>
 * </ul>
 *
 * @version 0.1
 */
...
public interface IElevator extends java.rmi.Remote {
	 ...
	 * @throws java.rmi.RemoteException - No Connection
	 */
